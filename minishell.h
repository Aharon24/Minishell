...
void	copy_without_quotes_and_expand(char *input, t_expand_data *d, t_env *env);
void	handle_dollar_or_char(char *input, t_expand_data *d, t_env *env);
int	handle_quotes(char *input, t_expand_data *d);
int	handle_dollar_case(char *input, t_expand_data *d, t_env *env, int *i);
...
int	ft_fork_and_manage(t_command *cmd, t_shell *shell, int pipefd[2], int *prev_fd);
